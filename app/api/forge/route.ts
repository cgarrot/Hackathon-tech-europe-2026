  ...
  try {
    ...
    if (providerResolution.type === "error") {
      return jsonError(providerResolution.message, 503, {
        hint: "Set LLM_PROVIDER=openai with OPENAI_API_KEY, or LLM_PROVIDER=ollama with OLLAMA_API_KEY + OLLAMA_BASE_URL. GameForge app runtime requires a real provider."
      });
    }
