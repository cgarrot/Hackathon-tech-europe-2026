...
export function validateForgeResult(result: ForgeResult): string[] {
  ...
  if (result.package.personas.some((persona) => persona.sampleLines.some((line) => {
    const tags = extractCompactBracketTags(line);
    return tags.some((tag) => !isAllowedEmotionTag(tag));
  }))) {
    issues.push("unsupported_persona_emotion_tag");
  }
