...
const DEFAULT_GAME_SESSION_RATE_LIMIT_MAX_REQUESTS = 120;
const DEFAULT_MAX_CONCURRENT_GAME_SESSION_REQUESTS = 8;
...
const gameSessionRateLimitBuckets = new Map<string, RateLimitBucket>();
let activeForgeRequests = 0;
let activeVoiceRequests = 0;
let activeGameSessionRequests = 0;
...
export function checkGameSessionRateLimit(clientId: string, now = Date.now()): RateLimitResult {
  return checkRateLimit(
    gameSessionRateLimitBuckets,
    clientId,
    "GAMEFORGE_SESSION_RATE_LIMIT_WINDOW_MS",
    "GAMEFORGE_SESSION_RATE_LIMIT_MAX_REQUESTS",
    DEFAULT_RATE_LIMIT_WINDOW_MS,
    DEFAULT_GAME_SESSION_RATE_LIMIT_MAX_REQUESTS,
    now
  );
}
...
export const gameSessionRateLimitHeaders = forgeRateLimitHeaders;
...
export function tryAcquireGameSessionRequestSlot() {
  const maxConcurrentRequests = readPositiveInteger(
    "GAMEFORGE_MAX_CONCURRENT_SESSION_REQUESTS",
    DEFAULT_MAX_CONCURRENT_GAME_SESSION_REQUESTS
  );

  if (activeGameSessionRequests >= maxConcurrentRequests) {
    return false;
  }

  activeGameSessionRequests += 1;
  return true;
}

export function releaseGameSessionRequestSlot() {
  activeGameSessionRequests = Math.max(0, activeGameSessionRequests - 1);
}

export function resetForgeRequestGuardsForTests() {
  forgeRateLimitBuckets.clear();
  voiceRateLimitBuckets.clear();
  gameSessionRateLimitBuckets.clear();
  activeForgeRequests = 0;
  activeVoiceRequests = 0;
  activeGameSessionRequests = 0;
}
