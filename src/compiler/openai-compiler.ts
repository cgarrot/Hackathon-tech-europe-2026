...
export async function compileWithLlmProvider(prompt: string, config: LlmProviderConfig): Promise<ForgeResult> {
  if (config.provider === "ollama") {
    return compileGuidedOllamaWithLlmProvider(prompt, config);
  }
  ...
  const routing = await runStructuredStage({
    config,
  ...
  const artifactPackage = await runStructuredStage({
    ...
    system: "You are the GameForge artifact generator. Create rules, cards, personas, visual asset prompts, Gradium-ready voice asset prompts, code stubs, acceptance tests, and a validation report. Every AI persona should have a fictional speechStyle, sampleLines, and a matching kind=voice asset prompt; do not imitate real living people. Use the selected pack validation rules. Generated file paths must be relative, must not be absolute, and must not contain traversal. Output strict schema only.",
...
async function compileGuidedOllamaWithLlmProvider(prompt: string, config: LlmProviderConfig): Promise<ForgeResult> {
